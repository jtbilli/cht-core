...
// 1) NEW change for issue #6784: add Safari check 
const isSafariBrowser = () => {
  const parser = window.bowser.getParser(window.navigator.userAgent);
  return parser.getBrowserName().toLowerCase() === 'safari';
}; // NEW WED
...
const checkUnsupportedBrowser = () => {
 ...
 // 2) NEW change for issue #6784: catch any Safari user first 
 if (isSafariBrowser()) {                                    
  const banner = document.getElementById('unsupported-browser');  
  // match the existing “outdated‑browser” wording pattern:
  banner.innerText = 'Let your administrator or supervisor know the' +
                      'currently supported browsers are Chrome or FireFox';
